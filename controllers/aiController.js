...
// controlling for enhancing a resume's job description
...
export const uploadResume = async (req, res) => {
  try {
    ...
    // Middleware se userId nikalna (Check if it's req.userId or req.user._id)
    ...
    const systemPrompt = "You are an expert AI Agent that extracts data from resumes into structured JSON.";

    // Schema definitions hata kar sirf keys rakhein
    ...
    Return ONLY a valid JSON object with this exact structure:
    {
      ...
      "project": [
        { "name": "", "type": "", "description": "", "is_current": false }
      ],
      "education": [
    ...
    const response = await ai.chat.completions.create({
      ...
      response_format: { type: 'json_object' } // Yeh sahi hai, AI ko JSON force karega
    });
    ...
    const newResume = await Resume.create({
      userId, 
      title: title || "Extracted Resume", 
      ...parsedData 
    });

    return res.status(201).json({ 
        message: "Resume extracted and saved!", 
